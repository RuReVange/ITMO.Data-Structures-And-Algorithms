...
int main() {
    ...
    for (pair<pair <int,int>, int> n : start_edges){
        if (flag == 0 && find_set(n.first.first, parent) == find_set(n.first.second, parent)){
            tmp = n;
            flag = 1;
        }
        if (find_set(n.first.first, parent) != find_set(n.first.second, parent)) {
    ...
    MST1 = globalWeight;
    globalWeight = tmp.second;

    for (int i = 1; i < n+1; ++i) {
        make_set(i, parent, size);
    }
    union_sets(tmp.first.first, tmp.first.second, parent, size);
